...
class komposisiSegmenController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Tentukan tahun mana yang akan diambil untuk ditampilkan
    ...
    public function runCalculationsForYear($tahun)
    {
        try {
            ...
            return redirect()->route('komposisi.index', ['tahun' => $tahun])->with('error', 'Terjadi error database saat perhitungan: ' . $e->getMessage());
        } catch (\Exception $e) {
            // Tangkap error umum lainnya (misal: kelas tidak ditemukan jika controller lain belum ada)
            return redirect()->route('komposisi.index', ['tahun' => $tahun])->with('error', 'Terjadi kesalahan sistem: ' . $e->getMessage());
        }

        return redirect()->route('komposisi.index', ['tahun' => $tahun])->with('success', 'Perhitungan data komposisi segmen berhasil diselesaikan untuk tahun ' . $tahun . '.');
    }
    ...
    public function simpanDataMerakIVA($tahun)
    {
        ...
        // redeemIVA sekarang mengambil dari golongan IVA
